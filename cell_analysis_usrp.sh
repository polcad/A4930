...
KALBIN="/home/przemek/bin/kalibrate-uhd/src/kal"
#GSMRECPATH="/opt/airprobe/gsm-receiver/src/python/"
#GSMRECPATH="/home/lenovo/tmp/gr-gsm/apps/"
GSMRECPATH="/home/przemek/bin/airprobe/gsm-receiver/src/python/"
...
then
        echo "Exit !!, ${GSMRECPATH}gsm_receive_usrp.py does not exist. Check the path or install the program."
        exit 0
fi
...
for band in ${BANDS}
do
	${KALBIN} -s ${band} -g $GAIN  &>> /tmp/out_kal.txt
done
...
then
	...
	do
		...
		offset=`echo "${linea}" | awk '{print $(NF-2)}' | sed 's/)//g'| sed 's/kHz//g' | sed 's/Hz//g' | sed 's/\.//g'`
		freq_final=`echo $[${freq_base} ${signo} ${offset}]`
		num_chan=0
