#!/bin/bash
# this script comes from Commotion iso
set -x
trap read debug
#if [ "$UID" -ne 0 ]; then
#  sudo "$0" "$@"
...
OPENBTS="/home/przemek/openbts/openbts"
...
UHDPATH=UHD-Fairwaves
BTSPATH=openbts-2.8
GRPATH=gnuradio-3.4.2
BTSPATHI=openbts
GRPATHI=gnuradio-3.4.2
UHDPATHI=UHDumtrx


install_gnuradio () {
  xterm -e "cd /home/przemek
  tar zxvf files/gnuradio-3.4.2-built.tar.gz
  cd gnuradio-3.4.2
  ./configure --prefix=$GPATH/gr342 -q --disable-gr-qtgui --with-fusb-tech=libusb1 --enable-usrp --enable-gr-usrp --enable-gnuradio-core --disable-usrp2 --disable-volk
  #make
  make install
  ...
  echo 'SUBSYSTEMS==\"usb\", ATTRS{idVendor}==\"fffe\", ATTRS{idProduct}==\"0002\", MODE:=\"0666\", GROUP:=\"usrp\"' > /etc/udev/rules.d/10-usrp.rules
  echo 'SUBSYSTEMS==\"usb\", ATTRS{idVendor}==\"2500\", ATTRS{idProduct}==\"0002\", MODE:=\"0666\", GROUP:=\"usrp\"' >> /etc/udev/rules.d/10-usrp.rules
  chown root:root /etc/udev/rules.d/10-usrp.rules"
}
  
install_uhd () {
  xterm -e "dpkg -i /home/przemek/files/uhd_003.005.001-release_i386.deb"
}
...
link_transceiver_RAD1 () {
  cd /home/przemek/openbts/openbts/trunk/apps
  make
...
#  sed -i -e s/openbts/$hostname/g /etc/hosts
  ...
  [ `/home/przemek/serval-dna/servald keyring list` ] || /home/przemek/serval-dna/servald keyring add
  SID=`/home/przemek/serval-dna/servald keyring list |cut -d ':' -f 1`
  /home/przemek/serval-dna/servald set did $SID $UUID $hostname

 # rm /home/przemek/.config/autostart/install-openbts.desktop
 # rm /home/przemek/Desktop/install-openbts.sh
 # mv /home/przemek/rc.local /etc/rc.local
 # sudo sed -i s/"openbts ALL=(ALL) NOPASSWD:ALL"/""/ /etc/sudoers
